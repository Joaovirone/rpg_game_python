...
def tick_efeitos_inicio_turno(alvo: Entidade) -> int:
    ...
    - eletro_turnos: 1d6-1 por turno (mín. 0)
    - veneno_turnos: dano fixo por turno (veneno_dano, padrão 2)
    - sangramento_turnos: tipo "d6" (rola 1d6/turno) ou fixo (sangramento_dano)
    - marca_fatal_turnos: 1d6 por turno
    - semente_turnos: quando zera, cura 1d20-5
    Também decrementa contadores de:
    - nao_pode_atacar, refletir_dano_turnos, invulneravel_turnos
    Retorna o dano total sofrido por efeitos nesse início de turno.
    """
...
def aplicar_buffs_de_ataque(user: "Personagem", dano_base: int) -> int:
    """
    Aplica bônus/crit do 'próximo ataque' e reseta os flags.
    """
...
class Personagem(Entidade):
    """
    Base dos personagens jogáveis. Subclasses definem os atributos base.
    """
    ...
    def calcular_dano_base(self) -> int:
        """
        Dano físico base: 1d6 + ataque (+ buffs/crit do próximo ataque).
        Bloqueado se 'nao_pode_atacar' > 0.
        """
    ...
    # -------- XP (nível máx. 10, libera especiais) --------
...
class Guerreiro(Personagem):
    """
    Vida 50, Ataque 8, Defesa 10, Mana 5, Ataque Mágico 0
    """
    ...
    # Habilidades anteriores (grupo “anteriores”)
    ...
    def usar_especial(self, n: int, alvo: Optional[Entidade] = None, **kwargs) -> int:
        # 1..7 (quatro novas + três anteriores)
...
class Mago(Personagem):
    """
    Vida 30, Ataque 1, Defesa 4, Mana 40, Ataque Mágico 10
    """
    def __init__(self, nome: str):
    ...
    def esp_eletrocussao(self, alvo: Entidade) -> int:
        # 2 mana | 3d6 agora + DoT 1d6-1 por 2 turnos
        if not self.gastar_mana(2):
            return 0
        dano = sum(d6() for _ in range(3))
        if hasattr(alvo, "efeitos"):
    ...
    def esp_explosao_florescente(self, alvo: Entidade) -> int:
        # 8 mana | 10d6 de dano e o mago NÃO age no próximo turno
        if not self.gastar_mana(8):
            return 0
...
class Arqueiro(Personagem):
    """
    Vida 35, Ataque 5, Defesa 4, Mana 25, Ataque Mágico 3
    """
    def __init__(self, nome: str):
    ...
    def esp_contaminar(self, alvo: Entidade) -> int:
        # 3 mana | envenena alvo (2 de dano/turno por 3 turnos)
        if not self.gastar_mana(3):
            return 0
        if hasattr(alvo, "efeitos"):
...
class Curandeiro(Personagem):
    """
    Vida 20, Ataque 0, Defesa 3, Mana 35, Ataque Mágico 8
    """
    def __init__(self, nome: str):
...
# 1) Mapa de arquétipos (nome -> classe)
...
def criar_personagem(nome_arquetipo: str, nome: str) -> Personagem:
    """
    Cria a instância do arquétipo informado.
    Se o nome não existir no mapa, cai em Guerreiro como fallback.
    """
...
def custo_ataque_basico(p: Personagem) -> int:
    """
    Custo de mana do ataque básico por classe.
    - Mago: 1
    - demais: 0
    """
...
def _num_especiais_desbloqueadas(nivel: int) -> int:
    """
    Desbloqueio progressivo até 7 especiais (além do ataque básico).
    Marcos: 1 (nível 1), +1 nos níveis 2, 4, 6, 8, 9 e 10 => total 7.
    """
    count = 1  # começa com 1 especial no nível 1
    for marco in (2, 4, 6, 8, 9, 10):
        if nivel >= marco:
            count += 1
    return min(7, count)
...
def especiais_do_personagem(p: Personagem, considerar_nivel: bool = True) -> list[tuple[int, str, int]]:
    """
    Retorna a lista de especiais (id, nome, custo). Agora com 7 entradas por classe.
    A UI/engine deve exibir até _num_especiais_desbloqueadas(nivel).
    """
...
def preview_personagem(p: Personagem) -> dict[str, int]:
    """
    Retorna dados principais para o HUD/preview.
    """
